...
def save_img_file_as_static_gif_with_alpha(img, transparent_gif_bg_path, save_filename, quantize):
    ...
    im.save(save_filename, transparency=transparency, version='GIF89a', optimize=True)
    return


def save_img_file_as_animated_gif_with_alpha(frame0, img, transparent_gif_bg_path, save_filename, version, duration):
    transparency = 255
    frame0.save(save_filename, transparency=transparency, save_all=True, optimize=True, append_images=img, disposal=2, version=version, loop=0, duration=duration)
    return
